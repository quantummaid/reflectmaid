...
import de.quantummaid.reflectmaid.typescanner.Report.Companion.failure
...
class Undetectable<T> constructor(context: Context<T>) : StatefulDefinition<T>(context) {
    ...
    override fun getDefinition(requirementsDescriber: RequirementsDescriber): Report<T> {
        ...
        val collectionResult: CollectionResult<T> = CollectionResult(detectionResult!!.result(), detectionRequirements)
        val mode = requirementsDescriber.describe(detectionRequirements)
        val reason = "no ${requirementsDescriber.describe(context.detectionRequirements())} detected:\n${
            context.detectionResult()!!.reasonForFailure()
        }"
        return failure(collectionResult, "unable to detect $mode:\n$reason")
    }
}